...
def _per_token_group_quant_fp8(
    ...
    for _ in tl.range(g_id, M, b_id_stride):
        ...
        y_ptr += b_id_stride * y_stride
        y_q_ptr += b_id_stride * y_stride
        y_s_ptr += b_id_stride



@triton.jit
def _per_token_group_quant_fp8_colmajor_v2(
    # Pointers to inputs and output
    y_ptr,
    y_q_ptr,
    y_s_ptr,
    ...
    for _ in tl.range(g_id, M, b_id_stride):
        scale_col = t_g_id % blocks_per_row
        scale_row = t_g_id // blocks_per_row
        ...
        tl.store(y_q_ptr + cols, y_q, mask=mask)
        tl.store(y_s_ptr, y_s)
        y_ptr += b_id_stride * group_size
        y_q_ptr += b_id_stride * group_size
        t_g_id += b_id_stride
...
def per_token_group_quant_fp8(
    ...
    column_major_scales_v2: bool = False,
    ...
    num_warps = 1
    props = get_device_props(x.device.index)
    num_sm = props['multi_processor_count']
    warps_per_sm = props['warps_per_sm']
    max_ctas = num_sm * warps_per_sm // num_warps
    grid_size = min(M, max_ctas)
    num_stages = 1
    grid = (grid_size, )
    if column_major_scales:
        if column_major_scales_v2 is False:
            num_warps = min(max(BLOCK // 256, 1), 8)
            ...
            _per_token_group_quant_fp8_colmajor_v2[grid](
                    x,
                    x_q,
                    x_s,
                    group_size,
